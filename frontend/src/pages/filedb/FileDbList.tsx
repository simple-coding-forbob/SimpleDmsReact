...
const FiledbList = () => {
  const [fileDb, setFileDb] = useState<IFileDb[]>([]);
  ...
  const remove = async (fid: number) => {
    try {
      await FileDbService.remove(fid);
      selectList();
    } catch (e) {
      console.log(e);
    }
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {fileDb.map((data) => (
          <div key={data.fid} className="border rounded shadow hover:shadow-md overflow-hidden">
